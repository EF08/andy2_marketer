...
| `scrape` | `{what: page\|thread\|comments\|profile\|feed, targetUrl\|handle, limit?, includeReplies?}` | `page` on all 5; the structured shapes are X |
| `search` | `{query, tab: latest\|top\|people, limit?}` | X — posts with permalinks + engagement counts |
| `post` | `{text}` | X (text only — no asset pipeline yet) |
| `reply` / `comment` | `{targetUrl, text}` | X (same act on X; they split when other platforms land) |
| `like` | `{targetUrl, undo?}` | X |
| `follow` | `{handle\|targetUrl, undo?}` | X |
| `dm` | `{handle, text}` | X (needs X Chat unlocked in the profile — see below) |
...
Instagram/TikTok/Facebook/YouTube can be read and session-checked today; acting on them needs their own adapters (and, for IG/TikTok posting, the asset pipeline from [MASTERPLAN.md](MASTERPLAN.md) §2.4).
...
- `src/executors/` — one module per action type, plus `xCommon.ts`: the shared X machinery (session lifecycle, login gate, interstitials, focal-tweet detection, tweet extraction, send confirmation, failure taxonomy). A new X action should be flow, not boilerplate.
...
X's DOM changes; the probe is how you check a selector against the live page before codifying it. Run it only while the agent is idle — both drive the same Chrome profile.
